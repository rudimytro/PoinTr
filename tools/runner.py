...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            sparse_loss = base_model.module.get_loss(ret, gt, epoch)
...
def validate(base_model, test_dataloader, epoch, ChamferDisL1, ChamferDisL2, val_writer, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if val_writer is not None and idx % 16 == 0:
                input_pc = partial.squeeze().detach().cpu().numpy()
                input_pc_img = misc.get_ptcloud_img(input_pc)
                val_writer.add_image('Model%02d/Input'% idx , input_pc_img, epoch, dataformats='HWC')
                ...
                total_pcd = np.concatenate((input_pc, sparse))[np.newaxis, ...]
                colors = np.zeros_like(total_pcd)
                colors[:, :input_pc.shape[0]] = [7, 87, 152]
                colors[:, input_pc.shape[0]:] = [0, 180, 110]
                val_writer.add_mesh('Model%02d/Pred_3D' % idx, total_pcd, colors=colors)
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if  'PCN' in dataset_name or 'ProjectShapeNet' in dataset_name:
                ...
                _metrics = Metrics.get(coarse_points, gt, require_emd=True)
